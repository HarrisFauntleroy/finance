...
const fetchAndParseJSON = async (url: URL) => {
  try {
...
export const updateMarketsCrypto = async () => {
  ...
  for (let page = pages; page > 0; page--) {
    /** Base url */
    const url = new URL(
      `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${baseCurrency}&order=market_cap_desc&per_page=${resultsPerPage}&page=${page}&sparkline=false`
    );
